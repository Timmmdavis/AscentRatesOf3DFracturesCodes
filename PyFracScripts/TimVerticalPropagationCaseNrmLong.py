...
# solid properties - Basaltic Dyke
nu = 0.25                           # Poisson's ratio       [dmlss] 
G= 25e9                             # Shear modulus         [pa] 
youngs_mod = (2*G)*(1+nu)           # Young's modulus       [pa] 
Eprime = youngs_mod / (1 - nu ** 2) # plain strain modulus  [pa]
K_IcReported = 6*1e6                        # fracture toughness    [pa.sqrt(m)]
K_Ic = 6*1e6                        # fracture toughness    [pa.sqrt(m)]
...
'''
# solid properties - Gelatin
nu = 0.5                           # Poisson's ratio       [dmlss] 
G= 276                             # Shear modulus         [pa] 
youngs_mod = (2*G)*(1+nu)           # Young's modulus       [pa] 
Eprime = youngs_mod / (1 - nu ** 2) # plain strain modulus  [pa]
K_IcReported = 19                        # fracture toughness    [pa.sqrt(m)]
K_Ic = 19                        # fracture toughness    [pa.sqrt(m)]
fluiddensity=1000-160                   # [kg/m3]
rockdensity=1000                    # [kg/m3]
deltagamma=(rockdensity-fluiddensity)*9.81 # gradient in weight [pa.m^-1]
eta=48e-3#1.8020e-5;                            # fluidviscosity [pa.s] - water=~1.1e-3
'''
...
rate=(volume/timeFor2c)*2#volume/lengthofinjection_s                  #[m/s] 

...
% t - time since start
% n - number of points (half the no you will get...)
%
% Outputs:
...
ztail=linspace(0,z,n);
%Eq.6.7a Roper and Lister
htail=sqrt((eta.*ztail)./(deltagamma.*t));

zhead=linspace(-c,c,n);
...
zheadbaseflag=zhead<0;
lowopening=hhead<h;
flag=lowopening+zheadbaseflag==2;
zmv=max(zhead(flag));
zhead(flag)=[];
hhead(flag)=[];

%Put in right place relative to each other
zpnts=[ztail,zhead+z+c-(c+zmv)];
hpnts=[htail,hhead];
